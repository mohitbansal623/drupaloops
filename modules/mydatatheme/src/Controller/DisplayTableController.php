...
//select records from table
    $query = \Drupal::database()->select('mydatatheme', 'm');
      $query->fields('m', ['id','name','mobilenumber','email','age','gender','website']);
      $results = $query->execute()->fetchAll();
        $rows=array();
    foreach($results as $data){
        $delete = Url::fromUserInput('/mydatatheme/form/delete/'.$data->id);
        $edit   = Url::fromUserInput('/mydatatheme/form/mydatatheme?num='.$data->id);
